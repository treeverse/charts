...
| `iqs.enabled`                         | Deploy IQS (Iceberg Query Service), an **internal** service. lakeFS Enterprise only (requires `enterprise.enabled`). Also sets the server's `metadata_search.query.enabled`, `endpoint` and `token`; `metadata_search.query.results_location` must still be set in `lakefsConfig`. | `false` |
