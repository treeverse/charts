...
| `service.trafficDistribution`         | [Traffic distribution](https://kubernetes.io/docs/concepts/services-networking/service/#traffic-distribution) preference for the lakeFS Service (e.g. `PreferClose` for topology-aware, zone-local routing). Requires Kubernetes 1.31+. When empty, the field is omitted.            |                                     |
...
| `iqs.enabled`                         | Deploy IQS (Iceberg Query Service), an **internal** service. lakeFS Enterprise only (requires `enterprise.enabled`). Also sets the server's `metadata_search.query.enabled`, `endpoint`, `token` and `results_location`. | `false` |
| `iqs.resultsLocation`                 | Required when `iqs.enabled`. Where lakeFS stores query results (`metadata_search.query.results_location`).                                                                                                                                                                          |                                     |
| `iqs.resultsStorageId`                | Blockstore id for `iqs.resultsLocation` (`metadata_search.query.results_storage_id`), for multi-blockstore setups.                                                                                                                                                                   |                                     |
| `iqs.podSecurityContext` / `iqs.securityContext` | Pod-level / container-level security context for IQS                                                                                                                                                                                                                  | `{}`                                |
| `iqs.resources`                       | IQS resource requests & limits. The memory limit also caps DuckDB's memory use.                                                                                                                                                                                                       | requests `500m`/`1Gi`, limit memory `2Gi` |
| `iqs.image.repository` / `iqs.image.tag` | IQS image                                                                                                                                                                                                                                                                        | `treeverse/lakefs-iqs` / `0.1.0` |
| `iqs.replicaCount`                    | Number of IQS pods                                                                                                                                                                                                                                                                   | `1`                                 |
| `iqs.extraEnvVars` / `iqs.extraEnvVarsSecret` | Extra env vars for IQS (`MDSQUERY_*` configure the service, e.g. `MDSQUERY_LOGGING_LEVEL`)                                                                                                                                                                                  |                                     |
| `secrets.iqsToken`                    | Bearer token shared by the lakeFS server and IQS. Required when `iqs.enabled` (ignored when existingSecret is set)                                                                                                                                                                    |                                     |
| `secretKeys.iqsToken`                 | Name of key in existing secret holding the IQS token. Only used when existingSecret is set.                                                                                                                                                                                          | `iqs_token`                         |
